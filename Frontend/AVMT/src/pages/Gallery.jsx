...
const AppleGalleryPro = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] pt-20 px-6">
      {/* TOP HEADING */}
      <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight mb-6">
        School Gallery
      </h1>

      {/* PREMIUM APPLE-LIKE FILTER TABS */}
      <div className="flex gap-3 mb-8 overflow-x-auto">
        {["all", "image", "video", "event"].map((t) => (
          <button
            key={t}
            onClick={() => setFilter(t)}
            className={`px-5 py-2 rounded-full font-semibold capitalize transition-all duration-300 whitespace-nowrap
              ${
                filter === t
      ...
      <div className="columns-2 sm:columns-3 md:columns-4 gap-4 space-y-4">
        {filteredList.map((item) => (
          <motion.div
            ...
            className="relative overflow-hidden rounded-2xl shadow-md hover:shadow-xl transition break-inside-avoid group cursor-pointer"
            onClick={() => openPreview(item)}
          >
            {/* TYPE BADGE */}
            <div className="absolute top-2 left-2 z-20 px-2 py-[2px] text-[10px] font-bold rounded-full bg-white/40 backdrop-blur-md text-gray-900 opacity-0 group-hover:opacity-100 transition">
              {item.type.toUpperCase()}
            </div>

            {/* Media */}
            {item.type === "video" ? (
              <video src={item.url} className="w-full rounded-xl" muted />
            ) : (
              <img src={item.url} className="w-full rounded-xl" loading="lazy" />
            )}
          </motion.div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {preview && (
          ...
          >
            ...
            >
              {/* BADGE INSIDE MODAL */}
              <div className="absolute top-4 left-4 z-50 px-3 py-1 text-xs font-bold rounded-full bg-black/40 backdrop-blur text-white">
              ...
              {/* Media Viewer */}
              {preview.type === "video" ? (
                <video src={preview.url} controls autoPlay className="w-full max-h-[80vh] object-contain" />
              ) : (
